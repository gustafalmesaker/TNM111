...
function display(svg, nodes, links) {
    ...
    function updateRightGraph(selectedNodes, selectedLinks) {
    ...
    function handleMouseOver(d, i) {
        ...
        const selectedNodes = [d];
        const selectedLinks = links.filter(l => l.source === d || l.target === d);

        // Update the right graph with selected nodes and links
        updateRightGraph(selectedNodes, selectedLinks);
    }
...
async function run() {
    ...
    display(rightGraph, [], []); // Display an empty graph initially
